...
# Script to run training and validation for different configurations
...
echo "Starting training and validation process at $(date)" | tee "$MAIN_LOG"
...
run_validation() {
    ...
    update_python_param "2nd_validation_train_limited_other_tuning.py" "MAX_TRAIN_ANNOTATIONS_PER_CATEGORY" "$max_train"
    update_python_param "2nd_validation_train_limited_other_tuning.py" "ft_type" "$ft_type"
    
    # Run validation
    echo "  Starting validation at $(date)..." | tee -a "$log_file"
    if python 2nd_validation_train_limited_other_tuning.py >> "$log_file" 2>&1; then
...
restore_python_script "2nd_validation_train_limited_other_tuning.py"
